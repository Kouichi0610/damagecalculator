...
func (s *raw) create(names []string, level, species uint) SpeedInfo {
	return newInfo(makeInfo(species, "補正なし0振り", names), stats.Raw(stats.NewLevel(level), stats.Species(species)))
}
func (s *raw4) create(names []string, level, species uint) SpeedInfo {
	return newInfo(makeInfo(species, "補正なし4振り", names), stats.Raw4(stats.NewLevel(level), stats.Species(species)))
}
func (s *raw252) create(names []string, level, species uint) SpeedInfo {
	return newInfo(makeInfo(species, "補正なし252振り", names), stats.Raw252(stats.NewLevel(level), stats.Species(species)))
}

func makeInfo(species uint, title string, names []string) string {
	res := fmt.Sprintf("%d族%s", species, title)
	cnt := 0
